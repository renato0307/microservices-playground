...
def handler(event, context):
    ...
    print(f"deleting tenant license: {event}")
    pass  # DO THE REAL WORK
    print(f"tenant license deleted: {event}")

    # send operation completion event
    event = TenantEvent(
        uuid=str(uuid.uuid4()),
        correlation_id=event['detail']['uuid'],
        source='DeleteTenatLicenseOperation',
        detail_type=DetailTypeEnum.delete_license_done.value,
